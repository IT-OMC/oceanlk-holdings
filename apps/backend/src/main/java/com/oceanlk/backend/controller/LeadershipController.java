        ...
        public ResponseEntity<?> createLeader(@RequestBody CorporateLeader leader, Principal principal,
                ...
                if (isSuperAdmin) {
                        ...
                        pendingChangeService.createApprovedChange(
                                        "CorporateLeader", savedLeader.getId(), "CREATE", principal.getName(),
                                        savedLeader, null);
                        ...
                        com.oceanlk.backend.model.PendingChange pendingChange = pendingChangeService
                                        .createPendingChange(
                                                        "CorporateLeader", null, "CREATE", principal.getName(), leader,
                                                        null);
                ...
                return repository.findById(id)
                                .map(existing -> {
                                        ...
                                        if (isSuperAdmin) {
                                                ...
                                                // Record in history for Super Admin
                                                pendingChangeService.createApprovedChange(
                                                                "CorporateLeader", id, "UPDATE", principal.getName(),
                                                                savedLeader, existingOriginal);

                                                // Log Action
                                                auditLogService.logAction(principal.getName(), "UPDATE",
                                                                "CorporateLeader", id,
                                                ...
                                                if (pendingChangeService.hasPendingChange(id)) {
                                                        return ResponseEntity.badRequest().body(Map.of(
                                                                        "error",
                                                                        "This member already has a pending change awaiting approval"));
                                                }

                                                leader.setId(id);
                                                com.oceanlk.backend.model.PendingChange pendingChange = pendingChangeService
                                                                .createPendingChange(
                                                                                "CorporateLeader", id, "UPDATE",
                                                                                principal.getName(), leader, existing);
                                                auditLogService.logAction(principal.getName(), "SUBMIT_FOR_APPROVAL",
                                                                "CorporateLeader", id,
                                                                "Submitted leadership member update for approval: "
                                                                                + leader.getName());
                                                return ResponseEntity.ok(Map.of(
                                                                "message",
                                                                "Leadership member update submitted for approval",
                                                                "pendingChange", pendingChange));
                                        }
                                })
                ...
                return repository.findById(id)
                                .map(leader -> {
                                        ...
                                        if (isSuperAdmin) {
                                                ...
                                                // Record in history for Super Admin
                                                pendingChangeService.createApprovedChange(
                                                                "CorporateLeader", id, "DELETE", principal.getName(),
                                                                leader, leader);

                                                // Log Action
                                                auditLogService.logAction(principal.getName(), "DELETE",
                                                                "CorporateLeader", id,
                                                ...
                                                com.oceanlk.backend.model.PendingChange pendingChange = pendingChangeService
                                                                .createPendingChange(
                                                                                "CorporateLeader", id, "DELETE",
                                                                                principal.getName(), leader, leader);
                                                auditLogService.logAction(principal.getName(), "SUBMIT_FOR_APPROVAL",
                                                                "CorporateLeader", id,
                                                                "Submitted leadership member deletion for approval");
                                                return ResponseEntity.ok(Map.of(
                                                                "message",
                                                                "Leadership member deletion submitted for approval",
                                                                "pendingChange", pendingChange));
                                        }
                                })
                                .orElse(ResponseEntity.notFound().build());
