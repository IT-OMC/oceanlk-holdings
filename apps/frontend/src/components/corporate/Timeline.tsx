...
import { Calendar, Anchor, Settings, Globe, Map, BookOpen, LucideIcon } from 'lucide-react'
...
function TimelineItem({ item, index }: { item: TimelineEntry; index: number }) {
  ...
  return (
    ...
    >
      ...
      <div className="w-5/12">
        <div
          className={`p-6 glass rounded-2xl border border-white/10 hover:border-accent/30 transition-all duration-300 transform hover:-translate-y-1 ${
  ...
  return (
    <div ref={containerRef} className="relative py-20 max-w-5xl mx-auto px-4">
      <div className="text-center mb-20">
        <h2 className="text-4xl font-bold mb-6 bg-clip-text text-transparent bg-gradient-to-r from-white via-blue-100 to-blue-200">
          Our Historic Journey
        </h2>
        <p className="text-gray-400 max-w-2xl mx-auto">
          From humble beginnings to a global ecosystem, trace the milestones that defined our path.
        </p>
      </div>
      ...
      <div className="relative pt-12">
        <div className="absolute left-1/2 transform -translate-x-1/2 w-1 h-full bg-blue-900/30 top-0 rounded-full overflow-hidden">
          <motion.div style={{ height }} className="w-full bg-gradient-to-b from-blue-500 via-accent to-blue-500" />
        </div>
