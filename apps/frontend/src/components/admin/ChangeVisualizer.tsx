...
const ChangeVisualizer: React.FC<ChangeVisualizerProps> = ({
    ...
    const regularFields = allKeys.filter(key => !mediaFields.includes(key));
    const presentMediaFields = allKeys.filter(key => mediaFields.includes(key));
    ...
    return (
        <div className="space-y-6">
            ...
            {presentMediaFields.length > 0 && (
                <div className="bg-[#0B1120] rounded-lg border border-gray-800 overflow-hidden shadow-inner p-6">
                    <div className="mb-4">
                        <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Media Preview</span>
                    </div>

                    {action === 'UPDATE' && oldData ? (
