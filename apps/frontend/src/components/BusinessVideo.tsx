...
export default function BusinessVideo() {
  ...
  const bgVideo1 =
    'https://www.youtube.com/embed/QRCJvp0p7uk?autoplay=1&mute=1&controls=0&loop=1&playlist=QRCJvp0p7uk&showinfo=0&rel=0&iv_load_policy=3&disablekb=1&playsinline=1'
  const bgVideo2 =
    'https://www.youtube.com/embed/6VfYIo2wBNk?autoplay=1&mute=1&controls=0&loop=1&playlist=6VfYIo2wBNk&showinfo=0&rel=0&iv_load_policy=3&disablekb=1&playsinline=1'
  ...
  useEffect(() => {
    const preloadTimer = setTimeout(() => setMountSecondVideo(true), 26000)
  ...
  return (
    <section ref={containerRef} className="relative min-h-screen overflow-hidden flex items-center justify-center bg-black">
      <motion.div style={{ y }} className="absolute inset-0 z-0 h-[120%]">
        <motion.div animate={{ opacity: showSecondVideo ? 0 : 1 }} transition={{ duration: 1 }} className="absolute inset-0 w-full h-full">
          <iframe
      ...
      <div className="relative z-20 container mx-auto px-6 lg:px-12 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center h-full">
        <motion.div style={{ opacity }} className="text-white space-y-6 max-w-xl">
          <motion.span
          ...
          >
            ...
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-300">Logistics</span>
          </motion.h2>
          ...
          >
            At OceanLK, we bridge continents through seamless supply chain solutions. Watch how we are redefining
            maritime and aviation logistics with innovation and sustainable practices.
          </motion.p>
          ...
          <motion.a
            ...
            whileTap={{ scale: 0.95 }}
            className="inline-block px-8 py-4 bg-white text-slate-900 font-bold rounded-lg shadow-lg hover:shadow-cyan-500/20 transition-all"
          >
