      ...
      <div className="relative z-10 container mx-auto px-6 h-[600px] flex flex-col lg:flex-row gap-4">
        <div className="lg:w-1/4 mb-10 lg:mb-0 flex flex-col justify-center pr-8 space-y-6">
          <h2 className="text-4xl font-bold">
            Our Business <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-300">Ecosystem</span>
          </h2>
          <p className="text-slate-400">
            Explore our integrated verticals designed to provide end-to-end supply chain mastery. Click to discover
            ...
            >
              ...
              <div className="absolute inset-0 p-6 flex flex-col justify-between z-10">
                <div className="flex justify-between items-start">
                  <span
                    className={`text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-br from-white to-white/50 ${
                      activeId === item.id ? '' : 'lg:-rotate-90 lg:mt-32 lg:whitespace-nowrap'
                    }`}
                  >
                    0{item.id}
                  </span>
                ...
                <div className="space-y-2">
                  <h3
                    className={`text-xl lg:text-3xl font-bold text-white whitespace-nowrap ${
                      activeId !== item.id ? 'lg:hidden' : ''
                    }`}
                  >
                  ...
                  {activeId === item.id && (
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
                      <p className="text-slate-200 text-sm lg:text-base max-w-lg mb-4">{item.description}</p>
                      <button
                        onClick={(e) => {
                          e.stopPropagation()
