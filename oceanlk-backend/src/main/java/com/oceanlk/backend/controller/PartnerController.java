                    ...
                    if (isSuperAdmin) {
                        ...
                        if (pendingChangeService.hasPendingChange(id)) {
                            return ResponseEntity.badRequest().body(Map.of(
                                    "error", "This partner already has a pending change awaiting approval"));
                        }

                        partner.setId(id); // Ensure ID matches
                        com.oceanlk.backend.model.PendingChange pendingChange = pendingChangeService
                                .createPendingChange(
                                        "Partner", id, "UPDATE", principal.getName(), partner, existing);
                        auditLogService.logAction(principal.getName(), "SUBMIT_FOR_APPROVAL", "Partner", id,
                                "Submitted partner update for approval: " + partner.getName());
                        return ResponseEntity.ok(Map.of(
        ...
        return repository.findById(id)
                .map(partner -> {
                    ...
                    if (isSuperAdmin) {
                        ...
                        com.oceanlk.backend.model.PendingChange pendingChange = pendingChangeService
                                .createPendingChange(
                                        "Partner", id, "DELETE", principal.getName(), partner, partner);
                        auditLogService.logAction(principal.getName(), "SUBMIT_FOR_APPROVAL", "Partner", id,
