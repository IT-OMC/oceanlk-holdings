                ...
                if (isSuperAdmin) {
                        ...
                        if (existing.isPresent()) {
                                ...
                                pendingChangeService.createApprovedChange(
                                                "PageContent", saved.getId(), "UPDATE", principal.getName(), saved,
                                                existingOriginal);
                                ...
                                // Record in history for Super Admin
                                pendingChangeService.createApprovedChange(
                                                "PageContent", saved.getId(), "CREATE", principal.getName(), saved,
                                                null);
                        ...
                        com.oceanlk.backend.model.PendingChange pendingChange = pendingChangeService
                                        .createPendingChange(
                                                        "PageContent", id, action, principal.getName(), content,
                                                        originalData);

                        auditLogService.logAction(principal.getName(), "SUBMIT_FOR_APPROVAL", "PageContent", id,
                                        "Submitted content " + action + " for page: " + content.getPageIdentifier()
                                                        + " section: "
