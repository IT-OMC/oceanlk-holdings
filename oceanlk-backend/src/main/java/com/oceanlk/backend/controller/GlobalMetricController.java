        ...
        public ResponseEntity<?> saveMetric(@RequestBody GlobalMetric metric, Principal principal,
                ...
                if (isSuperAdmin) {
                        ...
                        pendingChangeService.createApprovedChange(
                                        "GlobalMetric", savedMetric.getId(), "CREATE", principal.getName(), savedMetric,
                                        null);
                        ...
                        com.oceanlk.backend.model.PendingChange pendingChange = pendingChangeService
                                        .createPendingChange(
                                                        "GlobalMetric", null, "CREATE", principal.getName(), metric,
                                                        null);
                ...
                return repository.findById(id)
                                .map(existing -> {
                                        ...
                                        if (isSuperAdmin) {
                                                ...
                                                GlobalMetric existingOriginal = repository.findById(id).orElse(null);
                                                ...
                                                // Record in history for Super Admin
                                                pendingChangeService.createApprovedChange(
                                                                "GlobalMetric", id, "UPDATE", principal.getName(),
                                                                savedMetric, existingOriginal);

                                                auditLogService.logAction(principal.getName(), "UPDATE", "GlobalMetric",
                                                                id,
                                                ...
                                                if (pendingChangeService.hasPendingChange(id)) {
                                                        return ResponseEntity.badRequest().body(Map.of(
                                                                        "error",
                                                                        "This metric already has a pending change awaiting approval"));
                                                }

                                                metric.setId(id);
                                                com.oceanlk.backend.model.PendingChange pendingChange = pendingChangeService
                                                                .createPendingChange(
                                                                                "GlobalMetric", id, "UPDATE",
                                                                                principal.getName(), metric, existing);
                                                auditLogService.logAction(principal.getName(), "SUBMIT_FOR_APPROVAL",
                                                                "GlobalMetric", id,
                                                                "Submitted metric update for approval: "
                                                                                + metric.getLabel());
                                                return ResponseEntity.ok(Map.of(
                ...
                return repository.findById(id)
                                .map(metric -> {
                                        ...
                                        if (isSuperAdmin) {
                                                ...
                                                // Record in history for Super Admin
                                                pendingChangeService.createApprovedChange(
                                                                "GlobalMetric", id, "DELETE", principal.getName(),
                                                                metric, metric);

                                                auditLogService.logAction(principal.getName(), "DELETE", "GlobalMetric",
                                                                id,
                                                ...
                                                com.oceanlk.backend.model.PendingChange pendingChange = pendingChangeService
                                                                .createPendingChange(
                                                                                "GlobalMetric", id, "DELETE",
                                                                                principal.getName(), metric, metric);
                                                auditLogService.logAction(principal.getName(), "SUBMIT_FOR_APPROVAL",
                                                                "GlobalMetric", id,
